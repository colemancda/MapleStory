...
#
# Coverage is measured against the package's OWN, unit-testable library sources.
# Everything under the repo's `Sources/` directory counts, EXCEPT:
#   * the executable targets (MapleStoryClient83, MapleStoryServer28/62/83) —
#     thin `main`/argument-parsing wrappers that aren't unit tested;
#   * MapleStoryServer — requires a live MongoDB to exercise;
#   * MapleStoryClient — requires an SDL3 + OpenGL context to exercise;
#   * the C shim targets (CMapleStory, CZlibShim).
# Including those would only dilute the number with code that can't be unit
# tested without external infrastructure. Dependencies (checked out under
# `.build`), generated sources, and the test target are excluded as well, so the
# number reflects the coverage of this package's own testable library code.
#
...
#   COVERAGE_EXCLUDE_REGEX   Python regex matched against each source path; any
#                            file that matches is excluded from coverage. Defaults
#                            to the package's executable targets so only library
#                            code is measured.
...
# Non-unit-testable targets are excluded: the executable wrappers, the DB-backed
# server, the SDL/OpenGL client, and the C shims (see the header comment).
EXCLUDE_REGEX="${COVERAGE_EXCLUDE_REGEX:-/Sources/(MapleStoryClient83|MapleStoryClient|MapleStoryServer28|MapleStoryServer62|MapleStoryServer83|MapleStoryServer|CMapleStory|CZlibShim)/}"
...
# 4. Export an LCOV report (for Codecov / Coveralls / editors). Executable targets
#    are dropped here too so the LCOV matches the enforced number.
...
if [ -n "$TEST_BINARY" ] && [ -f "$PROFDATA" ]; then
    ...
    $LLVM_COV export \
        ...
        "$TEST_BINARY" \
        -ignore-filename-regex='.build/(checkouts|.*\.build)/|Tests/|\.derived/|DerivedSources/|Sources/MapleStoryClient83/|Sources/MapleStoryClient/|Sources/MapleStoryServer28/|Sources/MapleStoryServer62/|Sources/MapleStoryServer83/|Sources/MapleStoryServer/|Sources/CMapleStory/|Sources/CZlibShim/' \
        > "$OUTPUT"
else
...
echo "==> Computing coverage for ${SOURCE_PREFIX} (excluding executables)"
python3 - "$CODECOV_JSON" "$SOURCE_PREFIX" "$THRESHOLD" "$PWD" "$EXCLUDE_REGEX" <<'PY'
import json, os, re, sys

report_path, source_prefix, threshold, repo_root, exclude_regex = (
