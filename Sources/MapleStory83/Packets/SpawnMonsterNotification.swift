...
extension MobSpawnData {
    ...
    func encode(to container: MapleStoryEncodingContainer) throws {
        ...
        try container.encode(rx0, isLittleEndian: true)
        try container.encode(rx1, isLittleEndian: true)
        try container.encode(facing)
        try container.encode(UInt8(0))
        try container.encode(Int16(-1), isLittleEndian: true)
    }
}

// MARK: - SpawnMonsterNotification

/// Sent to clients on a map when a mob appears.
